...
\begin{table}
	...
	\begin{tabular}{|c|c|c|c|c|}
		\hline
		Name & Year & Image Type & Number & Metadata \\
		\hline
...
As demonstrated in \ref(diagnosis-number) the dataset is highly inbalanced based on the diagnosis of the skin lesion with 12,875 NV and 4,522 MEL. There are only 867 AK, where AK has very high variance compared to other skin lesions and the sample size is unlikely for adequate detection. Seborrhoeic keratosis (SK) is not in this dataset and it has many similarities to melanoma. Others such as DF, and VASC only contains a handful of images. The difference in image samples makes the dataset primarly useful for testing between NV and MEL.

\subsubsection{metadata}
In this dataset the images are accompanied by metadata. This metadata includes patients age, sex, and anatomical location. Analysing this data provides further insight into the influence of patient information has on the classification process. Most of all we're looking for similar numbers for each catagory and some cases might be removed from the classification process to improve results.
...
As demonstrated in figure \ref{age-number} the dataset is unbalanced relating to age and type of skin lesion. Variation might be because the skin lesions develop in older people more than younger. This could also mean that many of the skin lesions are developed and there is going to be unlikely to find un-developed skin lesion samples.
...
\subsection{Dermoscopic Structures}

ISIC 2017 that shares some images with ISIC 2019 has some additional metadata relating to the dermoscopic structure. This data was part of the tasks section and the data was linked manually to the ISIC 2019 using the image name. This includes 2,694 segmentation masks of pigment networks, negative networks, globules, milia-like cysts, and streaks making a total of 2,694 images. 


\subsubsection{ISIC 2020}
...
Figure \ref{ph2-diagnosis-number} dataset is highly unbalanced with only 40 melanoma images and 160 nevus images. The small number of samples makes this dataset ineffective for training neural network algorithms.
...
Demonstrated in figure \ref{ph2-asym}
...
Shown in figure \ref{ph2-colour-number} Light and dark brown appear more in typical and astypical nevus and white, blue, and black are more common in melanoma. There are only a handful of red colour samples.
